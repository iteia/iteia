...
class ConteudoHomeBO {
	...
	public function __construct($codformato, $qtd = 5) {
        ...
        $this->lista_usr_selecionados = $this->contdao->getUltimasDoFormato($this->codformato, $qtd);
	}
	...
	private function getTagCanal($contvo){
		return '<div class="tag-canal">'.
            Util::iif($contvo->getCodSubArea() == 0,
            Util::getHtmlCanal($contvo->getCodSegmento()),
            Util::getHtmlCanal($contvo->getCodSubArea())).
            //Util::getHtmlCanal($contvo->getCodSegmento()).
            '</div>';
	}
	...
	private function getHtmlVideo(&$contvo) {
        ...
        $html .= '<h1 class="midia"><a href="'.$contvo->getUrl().'" title="Ir para página deste conteúdo">'.htmlentities($contvo->getTitulo()).'</a></h1>';
		$html .= '<p>'.Util::cortaTexto(htmlentities($contvo->getDescricao()),200).'</p>';
    ...
    public function getListaUltimosAutores(){
        $ultimas = array();
        foreach($this->lista_usr_selecionados as $codconteudo){
            $contvo = $this->getConteudoVO($codconteudo);
            $ultimas[] = $contvo->getCodAutor();
            $ultimas[] = $contvo->getCodColaborador();
        }
