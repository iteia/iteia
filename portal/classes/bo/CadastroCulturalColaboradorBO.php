...
class CadastroCulturalColaboradorBO {
	...
	private function setDadosForm(&$dadosform) {
		...
		$this->dadosform['atuacao'] = strip_tags(trim($this->dadosform['atuacao']));
		...
		$this->dadosform['telefone'] = strip_tags(trim($this->dadosform['telefone']));
		$this->dadosform['finalendereco'] = Util::geraTags(substr(trim($this->dadosform['finalendereco']), 0, 30));
		
		// autor
		$this->dadosform['nome_autor'] = strip_tags(trim($this->dadosform['nome_autor']));
		$this->dadosform['nomeartistico_autor'] = strip_tags(trim($this->dadosform['nomeartistico_autor']));
		$this->dadosform['historico_autor'] = substr(strip_tags(trim($this->dadosform['historico_autor'])), 0, 1400);
		$this->dadosform['endereco_autor'] = strip_tags(trim($this->dadosform['endereco_autor']));
		$this->dadosform['complemento_autor'] = strip_tags(trim($this->dadosform['complemento_autor']));
		$this->dadosform['datanascimento_autor'] = strip_tags(trim($this->dadosform['datanascimento_autor']));
		$this->dadosform['codcidade_autor'] = (int)$this->dadosform['codcidade_autor'];
		$this->dadosform['codestado_autor'] = (int)$this->dadosform['codestado_autor'];
		$this->dadosform['codpais_autor'] = (int)$this->dadosform['codpais_autor'];
		$this->dadosform['cidade_autor'] = strip_tags(trim($this->dadosform['cidade_autor']));
		$this->dadosform['bairro_autor'] = strip_tags(trim($this->dadosform['bairro_autor']));
		$this->dadosform['codtipo_autor'] = (int)$this->dadosform['codtipo_autor'];
		$this->dadosform['numero_autor'] = strip_tags(trim($this->dadosform['numero_autor']));
		$this->dadosform['orgao_autor'] = strip_tags(trim($this->dadosform['orgao_autor']));
		$this->dadosform['email_autor'] = strip_tags(trim($this->dadosform['email_autor']));
		$this->dadosform['fone_autor'] = strip_tags(trim($this->dadosform['fone_autor']));
		$this->dadosform['finalendereco_autor'] = Util::geraTags(substr(trim($this->dadosform['finalendereco_autor']), 0, 30));
        
        $this->dadosform['senha_autor'] = Util::geraRandomico();
		//$this->dadosform['senha_autor'] = strip_tags(trim(stripslashes($this->dadosform['senha_autor'])));
		//$this->dadosform['senha2_autor'] = strip_tags(trim(stripslashes($this->dadosform['senha2_autor'])));
		$this->dadosform['deacordo_autor'] = intval($this->dadosform['deacordo_autor']);
	}
	...
	private function validaDadosForm() {
		...
		if (!$this->dadosform['codcidade']) $this->erro_campos[] = 'cidade';
		...
		if($this->dadosform['finalendereco'] == $this->dadosform['finalendereco_autor']){
			$this->erro_campos[] = 'finalendereco_autor';
			$this->erro_campos[] = 'finalendereco';
			$this->erro_mensagens[] = 'Endereço (URL) de <a href="#final_endereco"><strong>colaborador</strong></a> deve ser diferente de <a href="#final_endereco_autor"><strong>responsável</strong></a>';
		}
...
//        }
	...
	protected function setDadosVO() {
		...
		$this->colaboradorvo->setSituacao(1);
	}
	...
	protected function setDadosAutorVO() {
		...
		$this->autorvo->setNome($this->dadosform['nomeartistico_autor']);
