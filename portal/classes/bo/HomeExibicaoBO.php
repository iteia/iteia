...
class HomeExibicaoBO {
	...
	public function getNoticiaDestaque() {
		...
		$html .= '<small>'.date('d.m.Y - H\\hi', strtotime($contvo->getDataHora())).'</small> | <span class="tag-canal">'.(($contvo->getCodSubArea()==0)?(Util::getHtmlCanal($contvo->getCodSegmento())):(Util::getHtmlCanal($contvo->getCodSubArea()))).'</span><br />';
	...
	public function getHtmlConteudoCanais() {
		...
		foreach($canais as $key => $canal) {
			...
			$conteudos = $segdao->getCodConteudoPorCodSegmento($canal['cod_segmento'],10);
			...
			foreach($conteudos as $keya => $conteudo) {
				if($cont == 4)
					break;
                if($conteudo['cod_formato']>=6)
                    continue;
				$listaCanais[] = $conteudo;
				$cont++;
			}
			
			foreach($listaCanais as $keya => $conteudo) {
                if($conteudo['cod_formato']>=6)
                    continue;
				
				$contvo = $this->getConteudoVO($conteudo['cod_conteudo'], $conteudo['cod_formato']);
                
				$html .= '<li'.(!isset($listaCanais[$keya + 1]) ? ' class="no-border"' : '').'>';
				$html .= '<small>'.date('d.m.Y H\\hi', strtotime($contvo->getDataHora())).'</small>';
			...
			$html .= '</ul>';
			$html .= '<div class="todos"><a href="/busca_action.php?buscar=1&amp;canal='.$canal['cod_segmento'].'&amp;formatos=2,3,4,5,6,7" title="Ir para página deste canal"><strong>Ver todos</strong></a></div>';
			$html .= '</div>';
		}
		...
		if (count($eventos) > 1) {
			...
			foreach ($eventos as $i => $agenda) {
				if (isset($agenda["cod_conteudo"]))
					...
					$html .= "<li><small>".date("d/m - H:i", strtotime($agenda["data_inicial"]." ".$agenda["hora_inicial"]))."</small><br/><a href=\"/".$agenda["url_agenda"]."\">".Util::cortaTexto(htmlentities($agenda["titulo"]), 65)."</a></li>\n";
			}
			$html .= "</ul>\n";
		} else {
