...
if (!$editar) $videobo->setValorCampo('sessao_id', $sessao_id);
if ($codvideo && !$editar) $videobo->setDadosCamposEdicao($codvideo);
$permitir_comentarios = $videobo->getValorCampo("permitir_comentarios");
if (!$codvideo) $permitir_comentarios = true;
$codvideo = (int)$videobo->getValorCampo("codvideo");
$sessao_id = $videobo->getValorCampo('sessao_id');
$id_upload = mt_rand();
$item_menu = 'conteudo';
...
<script type="text/javascript" src="jscripts/jquery.progressbar.min.js"></script>
...
<form action="conteudo_edicao_video.php" method="post" enctype="multipart/form-data" name="form-video" id="form-video" onsubmit="$('#uploadprogressbar').show(); setTimeout('showUpload()', 1500);">
    <h3 class="titulo">Cadastro de V&iacute;deo</h3>
		<div class="box">
            ...
            <input type="hidden" name="videotemp" id="videotemp" value="<?=$videobo->getValorCampo('videotemp')?>" />
            <input type="hidden" name="videotemp_name" id="videotemp_name" value="<?=$videobo->getValorCampo('videotemp_name')?>" />
            <input type="hidden" name="videotemp_size" id="videotemp_size" value="<?=$videobo->getValorCampo('videotemp_size')?>" />
				...
				<label for="youtube">Cadastrar v&iacute;deo do <strong>youtube</strong> ou <strong>vimeo</strong></label><br />

				<div id="ytbox" class="display-none">
					<br /><label for="textfield4">Endere&ccedil;o do v&iacute;deo</label><br />
					<input type="text" name="link_video" value="<?=$videobo->getValorCampo("link_video")?>" class="txt" id="textfield4" <?=$videobo->verificaErroCampo("link_video")?> /><br />
					<small class="info">Ex.<strong> http://www.youtube.com/watch?v=XXXXXX</strong> ou <strong>http://vimeo.com/XXXXXXX</strong></small>
				</div>
				<div id="anexo">
					<br /><label for="fileField">Anexar v&iacute;deo*</label><br />
					<small>Voc&ecirc; pode fazer upload de arquivo flv, avi, wmv, asf e mpeg. (Tamanho m&aacute;ximo de 250MB)</small><br /><br />
					<input type="file" name="arquivo_video" id="fileField" size="45" <?=$videobo->verificaErroCampo("arquivo_video")?> />
					<br /><span class="progressbar" id="uploadprogressbar" style="display: none;">0%</span><img src="img/progressbar.gif" width="1" style="border:0px; background:#f4f4f4;" height="1" alt="" /><img src="img/progressbg_yellow.gif" style="border:0px; background:#f4f4f4;" width="1" height="1" alt="" /><br />
					<?php if ($videobo->getValorCampo("arquivo_video")):?>
					<div class="arquivo"><span><?=$videobo->getValorCampo('arquivo_original')?></span></div>
					<?php endif; ?>
                    <?php if ($videobo->getValorCampo("videotemp")):?>
                    <div class="arquivo"> <span><?=$videobo->getValorCampo("videotemp_name")?></span></div>
                    <?php endif; ?>
				</div>
				<?php else: ?>
				...
				<div class="box-imagem">
					<div class="visualizar-img" id="div_imagem_exibicao">
						<?php if ($videobo->getValorCampo('imgtemp')):?>
...
<?php
	...
	if ($videobo->getValorCampo("tipo") == 2):
		echo '$(".display-none").show(); $("#anexo").hide(); $("#youtube").attr("checked","checked");';
	endif;
endif;
?>
