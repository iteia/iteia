...
class BuscaCacheDAO {
	...
	public function setBuscaCache($id, $resultado, $total, $palavra, $autor, $expiracao) {
		if (count($resultado)) {
	...
	public function getBuscaConteudo(&$bdadosvo, &$sql_complemento) {
		...
		if (isset($param_extra['codcanal']) && $param_extra['codcanal'])
            $sql_where .= " AND (CO.cod_segmento = '".$param_extra['codcanal']."' OR CO.cod_subarea = '".$param_extra['codcanal']."')";
		...
		if (isset($param_extra['autor']) && $param_extra['autor'] && (!isset($param_extra['noticia_agenda'])))
			$sql_where .= " AND CO.cod_conteudo IN (SELECT cod_conteudo FROM Conteudo_Autores_Ficha WHERE cod_conteudo=CO.cod_conteudo AND cod_usuario = '".$param_extra['autor']."')";
                ...
                if (isset($param_extra['autor']) && $param_extra['autor'] && isset($param_extra['noticia_agenda']))
			$sql_where .= " AND CO.cod_autor = '".$param_extra['autor']."'";
		...
		if (isset($param_extra['direito']) && $param_extra['direito']) {
			switch($param_extra['direito']) {
				...
				case 1: $sql_where .= " AND CO.cod_licenca in (1,2,3,7,8)"; break;
				case 2: $sql_where .= " AND CO.cod_licenca in (1,2,4,5,7)"; break;
			}
		}
...
//print_r($bdadosvo->getListaFormatos());
...
//
//			if (isset($param_extra['ordenacao']) && $param_extra['ordenacao']) {
//				$sql2 = "SELECT COUNT(t1.cod_conteudo) AS total FROM Conteudo_Autores_Ficha AS t1, Conteudo AS t2 WHERE t2.excluido='0' AND t2.publicado='1' AND t2.situacao='1' AND t2.cod_formato IN (1,2,3,4) AND t2.cod_sistema='".ConfigVO::getCodSistema()."' AND t1.cod_usuario='".$sql_row['cod_usuario']."' AND t1.cod_conteudo=t2.cod_conteudo GROUP BY t1.cod_usuario";
...
//	}
	...
	public function getBuscaAutores(&$bdadosvo, &$sql_complemento) {
		...
		if ($bdadosvo->getPalavraChave())
			$sql_where .= " AND (VA.nome LIKE '%".$bdadosvo->getPalavraChave()."%' OR VA.descricao LIKE '%".$bdadosvo->getPalavraChave()."%'  OR VA.cidade LIKE '%".$bdadosvo->getPalavraChave()."%' OR VA.cod_usuario IN (SELECT cod_usuario FROM Autores WHERE nome_completo LIKE '%".$bdadosvo->getPalavraChave()."%'))";
		...
		if (isset($param_extra['paises']) && $param_extra['paises'])
			$sql_where .= " AND VA.cod_pais IN (".implode(', ', $param_extra['paises']).")";
	...
	public function getBuscaColaboradores(&$bdadosvo, &$sql_complemento) {
		...
		if ($bdadosvo->getPalavraChave())
			$sql_where .= " AND (VC.nome LIKE '%".$bdadosvo->getPalavraChave()."%' OR VC.descricao LIKE '%".$bdadosvo->getPalavraChave()."%' OR VC.cidade LIKE '%".$bdadosvo->getPalavraChave()."%')";
		...
		if (isset($param_extra['paises']) && $param_extra['paises'])
			$sql_where .= " AND VC.cod_pais IN (".implode(', ', $param_extra['paises']).")";
	...
	public function getResultadoBuscaUsuario(&$codusuario) {
		...
		$sql = "SELECT t1.*, t3.sigla, t3.cod_estado, t4.cod_cidade, t4.cidade, t5.titulo AS url_titulo, t6.nome_completo FROM Usuarios AS t1 LEFT JOIN Estados AS t3 ON (t1.cod_estado=t3.cod_estado) LEFT JOIN Cidades AS t4 ON (t1.cod_cidade=t4.cod_cidade) LEFT JOIN Urls AS t5 ON (t1.cod_usuario=t5.cod_item) LEFT JOIN Autores AS t6 ON (t1.cod_usuario=t6.cod_usuario) WHERE t1.cod_usuario='".$codusuario."' AND t5.tipo=t1.cod_tipo";
		$query = $this->banco->executaQuery($sql);
		return $this->banco->fetchArray($query);
	}
	...
	public function getResultadoBuscaCanais(&$codsegmento) {
		$dados = $this->banco->fetchArray($this->banco->executaQuery("SELECT CS.* FROM Conteudo_Segmento AS CS WHERE CS.cod_segmento='".$codsegmento."' AND CS.disponivel='1'"));
	...
	public function apagarCache($id) {
		$this->banco->sql_delete('Busca_Cache', "id='".$id."'");
	}
