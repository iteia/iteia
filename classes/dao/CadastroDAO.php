    ...
    public function getListaCadastros($dados, $inicial, $mostrar) {
		...
		switch ($tipo) {
			...
			case 2:
                $pg = 'cadastro_autores';
                if($buscarpor == 'wiki'){
                    $pg = 'cadastro_autores_wiki';
                }
            $paginalink = $pg.'.php?buscar='.$buscar.'&amp;palavrachave='.$palavrachave.'&amp;buscarpor='.$buscarpor.'&amp;situacao='.$situacao.'&amp;de='.$de.'&amp;ate='.$ate; break;
			...
			if ($palavrachave && $palavrachave != 'Buscar') {
				...
				switch($buscarpor) {
                    ...
                    case 'estado':
						...
						break;
					case 'wiki':
                        if($palavrachave == 'estado'){
                            $estado = " AND (t2.estado LIKE '".$palavrachave."%' OR t2.sigla LIKE '".$palavrachave."%')";
                        }
						$where .= " AND t1.cod_usuario IN (SELECT cod_usuario FROM Usuarios_Niveis WHERE cod_usuario=t1.cod_usuario AND nivel='1') AND (t1.nome LIKE '%".utf8_decode($palavrachave)."%' OR t1.cod_usuario IN (SELECT cod_usuario FROM Autores WHERE nome_completo LIKE '%".utf8_decode($palavrachave)."%')) $estado ";
						break;
					case 'autor':
					...
					case 'colaborador':
						...
						break;
					case 'administrador':
						$where .= " AND t1.cod_usuario IN (SELECT cod_usuario FROM Usuarios_Niveis WHERE cod_usuario=t1.cod_usuario AND (nivel='7' OR nivel='8'))";
						break;
				}
	...
	public function executaAcoes($acao, $codusuarios) {
		if ($acao) {
			switch($acao) {
                ...
                case 3: // desativar
					if (count($codusuarios)){
                        $query = "UPDATE Usuarios SET situacao='2' WHERE cod_usuario IN (".implode(',', $codusuarios).")";
						$this->banco->executaQuery($query);
                        $log = "insert into QueryLogs(ip_acesso,cod_usuario,descricao,cod_sistema) values('{$_SERVER['REMOTE_ADDR']}', '{$_SESSION['logado_dados']['cod']}', \"Usuarios desativados - ".implode(',', $codusuarios)."\",6)";
                        $this->banco->executaQuery($log);
                    }
				break;
			}
		}
